...
def DenseLSTM_models(input_nc,
              seqL=100,
               out_length=1,
               mode='denselstm_classi'):
    if mode == 'denselstm':
        return DenseLSTM(input_nc=input_nc, growth_rate=32, block_config=(2, 2, 4, 2),
                 num_init_features=400, bn_size=4, drop_rate=0.2, input_length=seqL)
    elif mode == 'denselstm_classi':
        return DenseLSTM_classi(input_nc=input_nc, growth_rate=32, block_config=(2, 2, 4, 2),
                num_init_features=400, bn_size=4, drop_rate=0.2, input_length=seqL)
...
class DenseLSTM(nn.Module):
    ...
    def __init__(self, input_nc=4, growth_rate=32, block_config=(2, 2, 4, 2),
        ...
        self.covs1 = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv1d(num_init_features, num_init_features, kernel_size=7, stride=1, padding=3, bias=False)),
            ('norm0', nn.BatchNorm1d(num_init_features)),
            ('relu0', nn.ReLU(inplace=True)),
            ('pool0', nn.MaxPool1d(kernel_size=3, stride=2, padding=1)),
        ]))
    ...
    def forward(self, x):
        ...
        # features0 = features0.permute(2, 0, 1)
...
class DenseLSTM_classi(nn.Module):
    r"""Densenet-BC model class, based on
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_
    ...
    def __init__(self, input_nc=4, growth_rate=32, block_config=(2, 2, 4, 2),
        ...
        super(DenseLSTM_classi, self).__init__()
        ...
        # Linear layer
        self.ratio = nn.Linear(int(length) * num_features, 2)
